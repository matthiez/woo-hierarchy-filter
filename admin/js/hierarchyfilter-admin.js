...
function handleEvent(a) {
    ...
    for (d in c) this.$$handleEvent = c[d], this.$$handleEvent(a) === !1 && (b = !1);
    return b
}
...
if (sorttable = {
        init: function () {
            ...
            if (0 == a.getElementsByTagName("thead").length && (the = document.createElement("thead"), the.appendChild(a.rows[0]), a.insertBefore(the, a.firstChild)), null == a.tHead && (a.tHead = a.getElementsByTagName("thead")[0]), 1 == a.tHead.rows.length) {
                sortbottomrows = [];
                for (b = 0; b < a.rows.length; b++) a.rows[b].className.search(/\bsortbottom\b/) != -1 && (sortbottomrows[sortbottomrows.length] = a.rows[b]);
                if (sortbottomrows) {
                ...
                for (b = 0; b < headrow.length; b++) headrow[b].className.match(/\bsorttable_nosort\b/) || (mtch = headrow[b].className.match(/\bsorttable_([a-z0-9]+)\b/), mtch && (override = mtch[1]), mtch && "function" == typeof sorttable["sort_" + override] ? headrow[b].sorttable_sortfunction = sorttable["sort_" + override] : headrow[b].sorttable_sortfunction = sorttable.guessType(a, b), headrow[b].sorttable_columnindex = b, headrow[b].sorttable_tbody = a.tBodies[0], dean_addEvent(headrow[b], "click", sorttable.innerSortFunction = function (a) {
                    ...
                    for (b = 0; b < rows.length; b++) row_array[row_array.length] = [sorttable.getInnerText(rows[b].cells[col]), rows[b]];
                    row_array.sort(this.sorttable_sortfunction), tb = this.sorttable_tbody;
                    for (b = 0; b < row_array.length; b++) tb.appendChild(row_array[b][1]);
                    delete row_array
                }))
            }
            ...
            for (c = 0; c < a.tBodies[0].rows.length; c++) if (text = sorttable.getInnerText(a.tBodies[0].rows[c].cells[b]), "" != text) {
            ...
            switch (a.nodeType) {
                ...
                case 1:
                case 11:
                    for (b = "", c = 0; c < a.childNodes.length; c++) b += sorttable.getInnerText(a.childNodes[c]);
            ...
            var b;
            newrows = [];
            for (b = 0; b < a.rows.length; b++) newrows[newrows.length] = a.rows[b];
            for (b = newrows.length - 1; b >= 0; b--) a.appendChild(newrows[b]);
            delete newrows
            ...
            var f;
            var g;
            for (c = 0, d = a.length - 1, e = !0; e;) {
                e = !1;
                for (f = c; f < d; ++f) if (b(a[f], a[f + 1]) > 0) {
                    g = a[f];
                    a[f] = a[f + 1], a[f + 1] = g, e = !0
                }
                if (d--, !e) break;
                for (f = d; f > c; --f) if (b(a[f], a[f - 1]) < 0) {
                    g = a[f];
                    a[f] = a[f - 1], a[f - 1] = g, e = !0
                }
                c++
...
window.onload = sorttable.init, dean_addEvent.guid = 1, fixEvent.preventDefault = function () {
    ...
    let d;
    for (d in a) "undefined" == typeof this.prototype[d] && b.call(c, a[d], d, a)
