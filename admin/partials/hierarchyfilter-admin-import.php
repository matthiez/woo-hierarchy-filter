...
if(isset($_FILES['hierarchyfilter_import_csv'])) {
	...
	if (is_wp_error($uploaded)) {
		...
		if ('csv' != pathinfo($file, PATHINFO_EXTENSION)) { die(__('The uploaded file needs the *.csv file extension.', 'hierarchyfilter'));}
		...
		$csv = fopen($file,"r");
		while(!feof($csv)) {
			if (count(fgetcsv($csv)) !== 3) { die(__('Column mismatch: Please check the format of the CSV.', 'hierarchyfilter')); }
		}
		fclose($csv);
		// load into database
