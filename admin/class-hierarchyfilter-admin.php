...
class HierarchyFilter_Admin
{
    ...
    public function enqueue_styles() {
        wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/hierarchyfilter-admin.css', [], $this->version, 'all');
    }

    public function enqueue_scripts() {
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/hierarchyfilter-admin.js', ['jquery'], $this->version, false);
    }
    ...
    public function hierarchyfilter_register_fields() {
        add_settings_field(
            ...
            [$this, 'text_callback'], // callback function
        ...
        add_settings_field(
            ...
            [$this, 'checkbox_callback'], // callback function
            'hierarchyfilter_settings', // menu page to display
        ...
        add_settings_field(
            ...
            [$this, 'select_callback'], // callback function
            'hierarchyfilter_settings', // menu page to display
            ...
            [
                ...
                'options' => ['value' => 'Danish', 'English', 'German']
            ]
        );
    }
    ...
    public function select_callback($args) {
        ...
        if (empty($atts['aria_label']) && !empty($atts['description'])) {
            $atts['aria_label'] = $atts['description'];
        }
        elseif (empty($atts['aria_label']) && !empty($atts['label'])) {
            $atts['aria_label'] = $atts['label'];
        }
    ...
    public function hierarchyfilter_editor_callback() {
        ...
        wp_editor($text, 'hierarchyfilter_text', $settings = []);
    }
    ...
    public function hierarchyfilter_meta_box_single_register() {
        add_meta_box('hierarchyfilter-meta-box-single', __('HierarchyFilter', 'hierarchyfilter'), [$this, 'hierarchyfilter_meta_box_single_content'], 'product', 'side', 'high', null);
    }
    ...
    public function hierarchyfilter_ajax_add_new() {
        ...
        if (!empty($_POST['hierarchyfilter_add_new_level_1'])) {
            $level_1 = sanitize_text_field($_POST['hierarchyfilter_add_new_level_1']);
        }
        if (!empty($_POST['hierarchyfilter_add_new_level_2'])) {
            $level_2 = sanitize_text_field($_POST['hierarchyfilter_add_new_level_2']);
        }
        if (!empty($_POST['hierarchyfilter_add_new_level_3'])) {
            $level_3 = sanitize_text_field($_POST['hierarchyfilter_add_new_level_3']);
        }
        if (empty($level_1) || empty($level_2) || empty($level_3)) {
            die(__('No field can be empty..', 'hierarchyfilter'));
        }
        global $wpdb;
        $table = $wpdb->prefix . "hierarchyfilter";
        if ($wpdb->insert($table, ['level_1' => $level_1, 'level_2' => $level_2, 'level_3' => $level_3]) === FALSE) echo '<strong>ERROR!</strong></br>';
    ...
    public function hierarchyfilter_ajax_admin_database_delete() {
        ...
        if (isset($_POST['hierarchyfilter_admin_database_delete'])) {
            ...
            if (count($ids) == 0) {
                die(__('No input values specified.', 'hierarchyfilter'));
            }
            foreach ($ids as $id) {
                $wpdb->prepare($wpdb->delete($table, ['id' => $id], ['%d']));
            }
            ?>
            <thead>
            <tr>
    ...
    public function hierarchyfilter_ajax_admin_database_edit() {
        ...
        foreach ($combined as $key => $value) $wpdb->update($table, ['level_1' => $value], ['id' => $key], ['%s'], ['%d']);
        ?>
        <thead>
        <tr>
            <th class="hierarchyfilter-admin-database-table-th"><?php _e('ID', 'hierarchyfilter'); ?></th>
        ...
        foreach ($table_data as $key => $row): ?>
            <tr>
                <td><?= $row->id ?></td>
                <td><input type='text' name='hierarchyfilter-edit-level-1[]' class='hierarchyfilter-edit-level-1' data-id='{$row->id}'
                           value='<?= $row->level_1 ?>'/></td>
                <input type='hidden' name='hierarchyfilter-edit-level-1-ids[]' value='<?= $row->id ?>'/>

                <td><input type='text' name='hierarchyfilter-edit-level-2[]' class='hierarchyfilter-edit-level-2' data-id='{$row->id}'
                           value='<?= $row->level_2 ?>'/></td>
                <input type='hidden' name='hierarchyfilter-edit-level-2-ids[]' value='<?= $row->id ?>'/>

                <td><input type='text' name='hierarchyfilter-edit-level-3[]' class='hierarchyfilter-edit-level-3' data-id='{$row->id}'
                           value='<?= $row->level_3 ?>'/></td>
    ...
    public function hierarchyfilter_ajax_admin_database_edit_level_1() {
        ...
        $wpdb->update($table, ['level_1' => $level_1], ['id' => $level_1_id], ['%s'], ['%d']);
    ...
    public function hierarchyfilter_ajax_admin_database_edit_level_2() {
        ...
        $wpdb->update($table, ['level_2' => $level_2], ['id' => $level_2_id], ['%s'], ['%d']);
    ...
    public function hierarchyfilter_ajax_admin_database_edit_level_3() {
        ...
        $wpdb->update($table, ['level_3' => $level_3], ['id' => $level_3_id], ['%s'], ['%d']);
