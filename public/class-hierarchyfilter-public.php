...
class HierarchyFilter_Public {
	...
	public function hierarchyfilter_remove_products_from_shop_page($query) {
		if (!$query->is_main_query() || !$query->is_post_type_archive() || is_admin()) { return; }
		if (isset($_COOKIE[get_option('hierarchyfilter_cookie_name')])) { $id = $_COOKIE[get_option('hierarchyfilter_cookie_name')]; }
	...
	public function hierarchyfilter_custom_product_tab($tabs) {
		if (1 == get_option('hierarchyfilter_product_tab')) {
			$tabs['hierarchy_filter'] = array(
				'title' 	=> __( get_option('hierarchyfilter_text_product_tab'), 'hierarchyfilter' ),
				'priority' 	=> 50,
				'callback' 	=> array($this, 'hierarchyfilter_custom_product_tab_callback')
			);
			return $tabs;
		}
		return false;
	...
	public function hierarchyfilter_custom_product_tab_callback() {
		...
		$data = array_map(function($level_1, $level_2, $level_3) { return array(get_option('hierarchyfilter_text_level_1') => $level_1, get_option('hierarchyfilter_text_level_2') => $level_2, get_option('hierarchyfilter_text_level_3') => $level_3); }, $levels_1, $levels_2, $levels_3);
		echo "<h2>" . get_option('hierarchyfilter_text_product_tab_header') . "</h2>";
		if (count($data) > 0) { ?>
			<table>
			<?php if (1 == get_option('hierarchyfilter_product_tab_thead')) {?>
				<thead><tr><th><?php echo implode('</th><th>', array_keys(current($data))); ?></th></tr></thead>
			<?php } ?>
				<tbody>
