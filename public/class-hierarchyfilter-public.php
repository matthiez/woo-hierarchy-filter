...
class HierarchyFilter_Public
{
    ...
    public function hierarchyfilter_custom_product_tab_callback() {
        ...
        foreach (get_post_meta(get_the_ID(), 'hierarchyfilter_assosciations', true) as $id) {
            $ids[] = $id;
        ...
        $data = array_map(function ($level_1, $level_2, $level_3) {
            return [get_option('hierarchyfilter_text_level_1') => $level_1,
                get_option('hierarchyfilter_text_level_2') => $level_2,
                get_option('hierarchyfilter_text_level_3') => $level_3];
        },
            array_map('current', $levels_1),
            array_map('current', $levels_2),
            array_map('current', $levels_3));
